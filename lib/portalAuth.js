...
function setSessionCookieHeaders(res, session) {
  if (!res || !session?.access_token) return;
  const nextCookies = createSessionCookies(session);
  const existing = res.getHeader?.("Set-Cookie");
  const merged = existing ? [...(Array.isArray(existing) ? existing : [existing]), ...nextCookies] : nextCookies;
...
async function refreshPortalSession(refreshToken, res) {
  if (!refreshToken) return null;
  const supabaseAuth = getSupabaseAuthClient();
  const { data, error } = await supabaseAuth.auth.refreshSession({ refresh_token: refreshToken });
  if (error || !data?.session || !data?.user) return null;
  setSessionCookieHeaders(res, data.session);
  return fetchProfileForAuthUser(data.user);
}

export async function getPortalSession(req, res) {
  ...
  try {
    if (token) {
      ...
      if (!error && data?.user) return await fetchProfileForAuthUser(data.user);
    }

    return await refreshPortalSession(cookies[REFRESH_COOKIE], res);
  } catch (error) {
...
export async function requirePortalSession(context, options = {}) {
  const user = await getPortalSession(context.req, context.res);
  if (!user) {

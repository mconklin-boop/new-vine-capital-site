...
async function listInvestorCommitments(user) {
  if (!user?.id) return [];
  try {
    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase
      .from("investor_commitments")
      .select("id, investor_id, deal_id, amount, funding_method, status, created_at, updated_at")
...
export async function getInvestorDashboardData(user) {
  const [deals, documents, updates, distributions, commitments] = await Promise.all([
    listInvestorDeals({ user }),
    listInvestorDocuments({ user }),
    listInvestorUpdates({ user }),
    listInvestorDistributions({ user }),
    listInvestorCommitments(user),
  ]);
  const totalCommittedCapital = commitments.reduce((sum, commitment) => sum + Number(commitment.amount || 0), 0);
  const totalFundedCapital = commitments.filter((commitment) => commitment.status === "Funded").reduce((sum, commitment) => sum + Number(commitment.amount || 0), 0);
  const activeCommitments = commitments.filter((commitment) => commitment.status === "Funded").length;
  const activeInvestments = new Set(commitments.filter((commitment) => commitment.status === "Funded").map((commitment) => commitment.deal_id)).size;
  const pendingDocuments = documents.filter((doc) => ["Needs Review", "Pending Upload"].includes(doc.status)).length;
  const pendingCommitments = commitments.filter((commitment) => ["Received", "Approved", "Pending Funding"].includes(commitment.status)).length;
  ...
  return {
    ...
    platformMetrics: {},
    platformTimeline: [],
    summary: deals.length || documents.length || distributions.length || commitments.length ? {

...
export async function listInvestorDeals({ fallback = true } = {}) {
  try {
    ...
    if (!data?.length && fallback) return mockDeals;
...
export async function listInvestorDocuments({ fallback = true } = {}) {
  try {
    ...
    if (error) throw error;
    if (!data?.length && fallback) return mockDocuments;
...
export async function listInvestorUpdates({ fallback = true } = {}) {
  try {
    ...
    if (error) throw error;
    if (!data?.length && fallback) return mockUpdates;
...
export async function listInvestorDistributions({ fallback = true } = {}) {
  try {
    ...
    if (error) throw error;
    if (!data?.length && fallback) return mockDistributions;
    return (data || []).map(normalizeDistribution);
  } catch (error) {

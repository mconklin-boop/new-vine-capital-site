...
const investmentTypeFilters = ["All", "Debt", "Equity", "Preferred Equity", "2nd Position", "1st Position", "EMD", "Appraisal", "Fund"];
...
export default function Opportunities({ user, deals = [] }) {
  ...
  return (
    <PortalLayout user={user} title="Active Investment Opportunities">
      ...
      {!!deals.length && <><section className="mt-6 border border-white/10 bg-white/5 p-4"><label className="grid gap-2 text-sm font-bold text-white/70">Status<select value={status} onChange={(event) => setStatus(event.target.value)} className="border border-white/10 bg-[#050605] px-4 py-3 text-white">{statuses.map((item) => <option key={item}>{item}</option>)}</select></label></section>
      <section className="mt-5 flex gap-2 overflow-x-auto pb-1 text-xs font-black uppercase tracking-wide text-white/65 [scrollbar-width:thin]">{investmentTypeFilters.map((item) => <button key={item} onClick={() => setType(item)} className={`shrink-0 border px-4 py-3 ${type === item ? "border-[#d5ad62] bg-[#d5ad62] text-[#11100b]" : "border-white/10 bg-white/5 hover:border-[#d5ad62] hover:text-[#d5ad62]"}`}>{item === "All" ? "All Opportunities" : item}</button>)}</section></>}

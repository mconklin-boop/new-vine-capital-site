...
function WhyInvestorsLike({ deal }) {
  ...
  return <Panel eyebrow="Investor Fit" title="Why Investors Like This Fund">{bullets.length ? <ul className="grid gap-3">{bullets.map((item) => <li key={item} className="border border-[#d5ad62]/20 bg-[#d5ad62]/10 p-3 text-sm leading-6 text-white/70">{item}</li>)}</ul> : <p>Investor fit notes have not been posted yet.</p>}</Panel>;
}

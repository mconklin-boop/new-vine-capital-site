...
import InstitutionalPlatformSections from "../components/InstitutionalPlatformSections";
  ...
  return (
    <div className="min-h-screen bg-[#050605] text-[#fffdf7]">
      ...
      <main>
        ...
        <section className="mx-auto max-w-7xl px-5 py-24 lg:px-8"><div className="mb-12 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#d5ad62]">Borrower Capital Solutions</p><h2 className="mt-4 max-w-3xl font-serif text-4xl leading-tight md:text-6xl">Capital for real-world deals.</h2></div></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">{programs.map(([kicker, title, copy]) => <article key={title} className="min-h-[275px] border border-white/10 bg-[#111613] p-7 transition hover:-translate-y-1 hover:border-[#d5ad62]/70"><span className="text-xs font-black uppercase tracking-[0.16em] text-[#d5ad62]">{kicker}</span><h3 className="mt-7 text-xl font-black">{title}</h3><p className="mt-4 text-sm leading-7 text-white/60">{copy}</p></article>)}</div></section>
        <InstitutionalPlatformSections />

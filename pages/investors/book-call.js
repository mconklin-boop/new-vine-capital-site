...
const preCallSteps = [
  ["01", "Complete the Investor Intake Form", "Submit background information so New Vine Capital can review fit, qualification, and suitability before an intake call."],
  ["02", "Book through HubSpot", "After the intake form, use the HubSpot meeting link provided in your JotForm flow to select a time."],
  ["03", "Return to New Vine confirmation", "After booking, the HubSpot flow can redirect back to the New Vine Capital confirmation page."],
];
  ...
  return (
    <div className="min-h-screen bg-[#050605] text-white">
      ...
      <main>
        ...
        <section className="bg-[#f4f1e9] px-5 py-20 text-[#050605] lg:px-8"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.8fr_1fr]"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#1f5b3f]">Before the Call</p><h2 className="mt-4 font-serif text-4xl leading-tight md:text-5xl">The intake form comes first.</h2><p className="mt-5 leading-8 text-[#53615a]">After submitting the investor intake form, investors can book through the HubSpot meeting link and return to the New Vine confirmation page.</p><Link href={siteLinks.callConfirmed} className="mt-7 inline-flex bg-[#1f5b3f] px-6 py-4 text-xs font-black uppercase text-white">View Booking Confirmation Page</Link></div><div className="grid gap-4">{preCallSteps.map(([number, title, copy]) => <article key={title} className="border-l-4 border-[#d5ad62] bg-white p-5"><span className="text-xs font-black text-[#1f5b3f]">{number}</span><h3 className="mt-2 text-xl font-black">{title}</h3><p className="mt-2 text-sm leading-6 text-[#53615a]">{copy}</p></article>)}</div></div></section>

        <section className="bg-white px-5 py-20 text-[#050605] lg:px-8"><div className="mx-auto max-w-4xl border-l-4 border-[#d5ad62] bg-[#f4f1e9] p-7"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#1f5b3f]">HubSpot Redirect URL</p><h2 className="mt-3 text-3xl font-black">Use the confirmation page after booking.</h2><p className="mt-4 leading-7 text-[#53615a]">Once your HubSpot meeting link is ready, set its post-booking redirect to the New Vine Capital confirmation page.</p><Link href={siteLinks.callConfirmed} className="mt-6 inline-flex w-full justify-center bg-[#d5ad62] px-6 py-4 text-xs font-black uppercase text-[#11100b] sm:w-auto">Open Confirmation Page</Link></div></section>

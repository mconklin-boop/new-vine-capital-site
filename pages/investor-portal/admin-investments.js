  ...
  return (
    <PortalLayout user={user} title="Investment Management">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Section eyebrow="Deal Documents" title="Upload Deal Room Document">
          <form onSubmit={uploadDocument} className="grid gap-4"><Field label="Deal"><select name="deal_id" required className={inputClass}>{deals.map((deal) => <option key={deal.id} value={deal.id}>{deal.name}</option>)}</select></Field><Field label="Document Name"><input name="name" required className={inputClass} /></Field><Field label="Document Type"><select name="type" className={inputClass}>{documentTypes.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Display Date"><input name="display_date" type="date" className={inputClass} /></Field><Field label="File"><input name="file" type="file" className={inputClass} /></Field><button disabled={busy} className="bg-[#d5ad62] px-5 py-3 text-xs font-black uppercase text-[#11100b] disabled:opacity-50">Upload Document</button></form>
        </Section>
